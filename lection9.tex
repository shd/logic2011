...
\begin{itemize}
\item Примитив $Z$ представит формула $Z (a, b) := (a=a \with b=0)$.
...
формулой $B (b,c,i,d) := \exists q ((b = q \cdot (1 + c \cdot (i+1)) + d) \with (d < 1 + c \cdot (i+1)))$
\end{lemma}
\begin{proof}Упражнение.\end{proof}
...
\begin{center}
$R(x_1, \dots x_{n+1}, a) := \exists b \exists c (\exists k (B (b,c,0,k) \with F (x_1,...x_n, k))$\\
       $\;\;\;\with B (b,c,x_{n+1},a)$\\
...
$M (x_1, \dots x_n,y) := F(x_1, \dots x_n,y,0) \with \forall z (z < y \rightarrow \neg F (x_1, \dots x_n,z,0))$
